...
struct node {
    ...
    node(std::variant<int, bool> value) {
        if(holds<bool>(value)) {
            this->type = node_type::boolean;
            this->val = std::get<bool>(value);
        } else {
            this->type = node_type::integer;
            this->val = std::get<int>(value);
        }
    }

    std::variant<int, bool> eval(const std::map<std::string, std::shared_ptr<node>>& = {}) const;
};
...
U series_operation(std::vector<std::shared_ptr<node>> operands, T init, binaryFunction b, const variable_map& variables) {
    ...
    for(const auto& operand : operands) {
        if(auto res = operand->eval(variables); holds<T>(res))
            init = b(init, std::get<T>(res));
        else error_type();
    }
    return init;
}
...
U single_operation(std::vector<std::shared_ptr<node>> operands, binaryFunction b, const variable_map& variables) {
    ...
    if(holds<T>(op1) && holds<T>(op2))
        return b(std::get<T>(op1), std::get<T>(op2));
    else error_type();
}
